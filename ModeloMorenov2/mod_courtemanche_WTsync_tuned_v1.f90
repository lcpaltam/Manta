...
!------------------------------------------------------------------------------!
  ...
  select case (tcell)
  case (Courte_mod)
    v_prm = (/ 15.0_rp, p_gCaL, p_gto, p_gKur, p_gKr, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_RA/)
  case (Courte_mod_RA_PM)
    v_prm = (/ 15.0_rp, p_gCaL, p_gto, p_gKur, p_gKr, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_RA/)
  case (Courte_mod_CT_BBra)
    v_prm = (/ 15.0_rp, p_gCaL_CT_BBra, p_gto, p_gKur, p_gKr, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_RA/)
  case (Courte_mod_TVR)
    v_prm = (/ 15.0_rp, p_gCaL_TVR, p_gto, p_gKur, p_gKr_TVR, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_RA/)
  case (Courte_mod_RAA)
    v_prm = (/ 15.0_rp, p_gCaL_RAA, p_gto_RAA, p_gKur, p_gKr_RAA, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_RA/)
  case (Courte_mod_LA)
    v_prm = (/ 15.0_rp, p_gCaL_LA, p_gto, p_gKur, p_gKr_LA, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_LA/)
  case (Courte_mod_PV)
    v_prm = (/ 15.0_rp, p_gCaL_PV, p_gto_PV, p_gKur, p_gKr_PV, p_gKs_PV, p_gK1_PV, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_LA/)
  case (Courte_mod_LAA)
    v_prm = (/ 15.0_rp, p_gCaL_LAA, p_gto_LAA, p_gKur, p_gKr_LAA, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_LA/)   
  case (Courte_mod_MVR)
    v_prm = (/ 15.0_rp, p_gCaL_MVR, p_gto, p_gKur, p_gKr_MVR, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_LA/)
  case (Courte_mod_BBla)
    v_prm = (/ 15.0_rp, p_gCaL_BBla, p_gto, p_gKur, p_gKr_BBla, p_gKs, p_gK1, &
               p_gbCa, p_gbNa,p_ACh,p_IKACh_LA/)
  case default
    write (*,10) tcell; stop
  end select
...
! ===================================================================================== 
  ...
  ! --- Ajuste a WT: se eliminan escalados x*, flec/verna y se usan directamente
  !     las tasas del modelo WT_Flec_Implicit.h (WT Fits Reduced Model).
  actshift = -20.0_rp
  h1       = 0.0_rp

  ! Concentración de fármaco (se reutiliza la variable global existente)
  D_verna = p_verna_conc_nM
  conc    = D_verna * 1.0e-9_rp
  ...
  ! Tasas WT
  a11 = Tfactor*8.5539_rp/(7.4392e-2_rp*safe_exp(-Vm/17.0_rp) + 2.0373e-1_rp*safe_exp(-Vm/150.0_rp))
  a12 = Tfactor*8.5539_rp/(7.4392e-2_rp*safe_exp(-Vm/15.0_rp) + 2.0373e-1_rp*safe_exp(-Vm/150.0_rp))
  a13 = Tfactor*8.5539_rp/(7.4392e-2_rp*safe_exp(-Vm/12.0_rp) + 2.0373e-1_rp*safe_exp(-Vm/150.0_rp))
  b11 = Tfactor*7.5215e-2_rp*safe_exp(-Vm/20.3_rp)
  b12 = Tfactor*2.7574_rp*safe_exp(-(Vm-5.0_rp)/20.3_rp)
  b13 = Tfactor*4.7755e-1_rp*safe_exp(-(Vm-10.0_rp)/20.3_rp)

  a3 = Tfactor*5.1458e-6_rp*safe_exp(-Vm/8.2471_rp)
  b3 = Tfactor*6.1205_rp*safe_exp(Vm/13.542_rp)

  a2 = 0.25_rp*Tfactor*13.370_rp*safe_exp(Vm/43.749_rp)
  b2 = (a13*a2*a3)/(b13*b3)

  ax = 3.4229e-2_rp*a2
  bx = 1.7898e-2_rp*a3

  ! Cinética de unión WT
  kd_open = 11.2e-6_rp*safe_exp(-0.7_rp*Vm*p_F/(p_R*p_T))
  ...
  k_off = 400.0e-6_rp * diffusion
  ki_on = k_on/2.0_rp
  ki_off= 5.4e-6_rp * diffusion
  kc_on = k_on/2.0_rp
  kc_off= 800.0e-6_rp * diffusion

  ax1  = 5.7839e-5_rp * ax
  bx1  = 1.6689e-8_rp * bx
  a13c = 3.6324e-3_rp * a13
  a22  = 1.4847e3_rp  * a2
  b33  = 1.7352e-6_rp * b3
  a33  = 6.7505e-5_rp * a3
  a44  = 2.4135_rp    * a2
  ...
  a_22 = 4.2385e1_rp  * a2
  b_33 = 2.1181_rp    * b3
  a_44 = 1.0326e-3_rp * a2
  b_44 = 2.1378e-2_rp * a3

  if (conc <= 0.0_rp .or. conc_dplus <= 0.0_rp) then
    b13c = 0.0_rp
  else
  ...
  if (conc <= 0.0_rp .or. conc_d <= 0.0_rp) then
    a_33 = 0.0_rp
  else
